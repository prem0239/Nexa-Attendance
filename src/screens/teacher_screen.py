...
def teacher_tab_take_attendance():
    ...
    with c2:
        ...
        if st.button('Run Face Analysis', width='stretch',type='secondary', icon=':material/delete:', disabled=not has_photos ):
            with st.spinner('Deep Scaning classroom photos....'):
                all_detected_ids = {}
...
def reigster_teacher(teacher_username, teacher_name, teacher_pass, teacher_pass_confirm):
    if not teacher_username or not teacher_name or not teacher_pass:
        return False, "All fields are Required"
    if check_teacher_exists(teacher_username):
        return False, "Username is already take"
...
def teacher_screen_register():
    ...
    with c1:
        header_dashboard()
    with c2:
        if st.button("Go Back To Home", type='secondary', key='logininbackbtn', shortcut="control+backspace"):
            st.session_state['login_type'] = None
            st.rerun()
    ...
    teacher_username = st.text_input("Enter Username", placeholder="Usernmae")

    teacher_name = st.text_input("Enter Name", placeholder='Your Name')

    teacher_pass = st.text_input("Enter Password", type='password', placeholder="Enter the Password")
    ...
    with btnc1:
        if st.button('Register now',icon=':material/passkey:', shortcut='control+enter', width='stretch'):
            success, message = reigster_teacher(teacher_username, teacher_name, teacher_pass, teacher_pass_confirm)
            if success:
                st.success(message)
                import time
                time.sleep(2)
